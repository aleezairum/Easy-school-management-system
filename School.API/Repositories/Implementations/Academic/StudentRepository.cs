    ...
    public class StudentRepository : IStudentRepository
    {
        ...
        public async Task<ResponseDto> SaveAsync(StudentSaveDto dto, int userId, string userIp)
        {
            ...
            await _context.Database.ExecuteSqlRawAsync(
                "EXEC SpSave_SMSStudent @StudentID, @FatherName, @FatherNameUrdu, @FatherCNIC, @FatherMobile, @IsFatherSMS, @Occupation, @MotherName, @MotherNameUrdu, @MotherCNIC, @MotherMobile, @IsMotherSMS, @GuardianType, @GuardianName, @GuardianNameUrdu, @GuardianCNIC, @GuardianMobile, @IsGuardianSMS, @GuardianRelation, @RegistrationNo, @AdmissionNo, @AdmissionDate, @Name, @Name_Urdu, @MobileNo, @IsSMS, @Gender, @DOB, @BirthPlace, @FormBNo, @Religion, @Caste, @Address, @SessionID, @SectionID, @RollNo, @GradeID, @StatusID, @IsAvailAcademy, @UserID, @UserIP",
                parameters);
            ...
            return new ResponseDto
            {
                VID = dto.VID,
                ReturnCode = 0,
                ReturnMessage = dto.VID > 0 ? "Student updated successfully" : "Student saved successfully"
            };
        }
    }
}
