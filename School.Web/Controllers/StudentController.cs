        ...
        public async Task<IActionResult> Save(StudentFormViewModel model)
        {
            try
            {
                ...
                var studentData = new
                {
                    ...
                    guardianIsSMS = model.GuardianIsSMS,
                    ...
                    monthlyFee = string.IsNullOrEmpty(model.MonthlyFee) ? 0m : decimal.Parse(model.MonthlyFee),
                    admissionFee = string.IsNullOrEmpty(model.AdmissionFee) ? 0m : decimal.Parse(model.AdmissionFee),
