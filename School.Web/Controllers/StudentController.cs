    ...
    public class StudentController : Controller
    {
        ...
        public async Task<IActionResult> Save(StudentFormViewModel model)
        {
            try
            {
                ...
                // Prepare the data for API (property names must match StudentSaveDto)
                var studentData = new
                {
                    vID = model.Id,
                    parentID = 0,
                    ...
                    admissionDate = string.IsNullOrEmpty(model.DateOfAdmission) ? (DateTime?)null : DateTime.Parse(model.DateOfAdmission),
                    rollNo = model.RollNo ?? "",
                    name = model.NameOfStudent ?? "",
                    name_Urdu = model.NameOfStudentUrdu ?? "",
                    mobileNo = model.MobileNo ?? "",
                    isSMS = model.IsSMS,
                    gender = model.Gender ?? "",
                    dOB = string.IsNullOrEmpty(model.DateOfBirth) ? (DateTime?)null : DateTime.Parse(model.DateOfBirth),
                    birthPlace = model.PlaceOfBirth ?? "",
                    formBNo = model.FormBNo ?? "",
                    religion = model.Religion ?? "",
                    caste = model.Nationality ?? "",
                    ...
                    isFatherSMS = model.FatherIsSMS,
                    occupation = model.FatherOccupation ?? "",
                    motherName = model.MotherName ?? "",
                    motherNameUrdu = model.MotherNameUrdu ?? "",
                    motherCNIC = model.MotherCNIC ?? "",
                    motherMobile = model.MotherMobile ?? "",
                    isMotherSMS = model.MotherIsSMS,
                    guardianType = model.GuardianType,
                    guardianName = model.GuardianName ?? "",
                    guardianNameUrdu = model.GuardianNameUrdu ?? "",
                    guardianCNIC = model.GuardianCNIC ?? "",
                    guardianMobile = model.GuardianMobile ?? "",
                    isGuardianSMS = model.GuardianIsSMS,
                    guardianRelation = model.GuardianRelation ?? "",
                    sessionID = 0,
                    sectionID = 0,
                    classRollNo = "",
                    fee = string.IsNullOrEmpty(model.MonthlyFee) ? 0m : decimal.Parse(model.MonthlyFee),
                    gradeName = "",
                    gradeID = (int?)null,
                    statusID = (int?)null,
                    isAvailAcademy = false,
